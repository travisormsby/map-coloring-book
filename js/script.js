...
function colorSelect (colorClass) {
  // eliminate highlighting on the previous uncolored selection
  let deselected = svgObject.querySelectorAll (
    `.${colorChoice}[fill="url(#OvalPattern)"]`
  );
...
function colorContrast (inColor) {
  if (inColor.slice (0, 3) === 'rgb') {
    var vals = inColor.slice (4, -1).split (',');
    var red = vals[0];
    var green = vals[1];
    var blue = vals[2];
  } else {
    var vals = inColor.slice (1);
    var red = parseInt (vals.slice (0, 2), 16);
    var green = parseInt (vals.slice (2, 4), 16);
    var blue = parseInt (vals.slice (4, 6), 16);
  }
  var brightness =
